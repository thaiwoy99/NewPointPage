...
import { MobileProtocolCard } from "@/components/incentive-hub/MobileProtocolCard";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [walletAddress, connecting, connected, twitterUsername]);
  ...
  return (
    <div className="w-full flex flex-col px-6 py-2 mt-32">
      ...
      <main className="px-1 py-6 sm:px-6 sm:py-6">
        <h2
          id="eclipse-dapps"
        ...
        {loading ? (
          <div className="col-span-full flex justify-center">
            ...
            <div className="w-full grid grid-cols-2 gap-6 mt-12 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
              {data.map((protocol) => (
                <ProtocolCard key={protocol.name} {...protocol} twitterUsername={twitterUsername ?? null} />
              ))}
                </div>
                </div>
            <div className="block sm:hidden w-full grid grid-cols-2 gap-6 mt-12 sm:grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
              {data.map((protocol) => (
                <MobileProtocolCard key={protocol.name} {...protocol} />
              ))}
            </div>
          </>
        )}
